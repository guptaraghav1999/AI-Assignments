...
def depthFirstSearch(problem):
    ...
    while not frontier.isEmpty():
        ...
        if not currState[0] in vis:            
            if problem.isGoalState(currState[0]):
                return currState[1]

            vis[currState[0]] = True
            # vis.append(currState[0])
            successors = problem.getSuccessors(currState[0])    
            for node in successors:
                if not node[0] in vis:
                    temp = currState[1].copy()
                    temp.append(node[1])
                    frontier.push((node[0], temp))
                ...
                if not node[0] in vis:
                    ...
                    for index, (p, c, i) in enumerate(frontier.heap):
                        if i[0] == node[0]:
                            ...
                            frontier.heap.append((priority, c, (node[0], temp, currState[2]+node[2])))
                            heapq.heapify(frontier.heap)
                            break
                    else:
                        frontier.push((node[0], temp, currState[2]+node[2]), priority) 
        ...
        if not currState[0] in vis:            
            ...
            for node in successors:
                if not node[0] in vis:
                    temp = currState[1].copy()
                    temp.append(node[1])
                    priority = currState[2]+node[2]+heuristic(node[0], problem)
